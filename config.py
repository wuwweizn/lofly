...
DATA_SOURCE = {
    ...
    # 基金列表数据源配置
    # 注意：Tushare token 现在通过UI配置，不再硬编码
    # 首次使用时，请在"数据源配置"中设置Tushare token
    'fund_list_sources': {
        'tushare': {'enabled': True, 'priority': 1, 'name': 'Tushare', 'token': ''},
    ...
    # 向后兼容的旧配置
    # 注意：tushare_token 现在通过UI配置或环境变量设置
    # 优先使用环境变量 TUSHARE_TOKEN，其次使用UI配置
    'use_tushare': True,
    'tushare_token': '',  # 从环境变量或UI配置中读取
}
